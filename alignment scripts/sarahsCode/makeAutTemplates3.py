## makeAutTemplates3.py last updated by Sarah Ewing 11/28/18
## This file should take the gold alignments from the goldalign-repo-master file
## and make paraphrase templates from them.  Version three will incorporate POS
## tags in one-to-one alignments and will add # of words in each variable
...
stopwords = {'is', 'are', 'what', 'who', 'where', 'when', 'how', 'why', 'you', 'your', 'have', 'has', 'be', 'to',
             'them'}
                ...
                if line[6] == '1':
                    ...
                    for i in range(0, len(finalaligns)):
                        ...
                        if len(left) == 1 and len(right) == 1:
                            ...
                            while j < len(left):
                                if j == 0:
                                    sent1a[left[j]] = "$" + str(variable) + '/' + len(left)
                                    j += 1
                                else:
                            ...
                            while k < len(right):
                                if k == 0:
                                    sent2a[right[k]] = "$" + str(variable) + '/' + len(right)
                                    k += 1
                                else:
...
pickle.dump(templates, open('autTemplates3.p', 'wb'))